...
-- 1. 함수 하나 이상의 PL/SQL 문으로 구성된 서브루틴으로
...
FROM DUAL;
--==>> 20.08177733597172441258462763839107925133
...
CREATE OR REPLACE FUNCTION FN_WORKYEAR(VIBSDATE DATE)
RETURN VARCHAR2
IS 
    VRESULT VARCHAR2(20);
BEGIN
    VRESULT := TRUNC(MONTHS_BETWEEN(SYSDATE, VIBSDATE)/12)||'년'||
               TRUNC(MOD(MONTHS_BETWEEN(SYSDATE, VIBSDATE), 12))||'개월';
...
-- 1. PL/SQL 에서 가장 대표적인 구조인 스토어드 프로시더는
