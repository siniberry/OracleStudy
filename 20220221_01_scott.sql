...
--   하지만, 맨 위의 쿼리문(OR) 이 가장 빠르게 처리 된다.
...
--※ 시은아~ 오타가 자주 발생할 때에는... 띄어쓰기도 신경써서 해야한당~
...
--○ TBL_SASWON 테이블에서 이름의 마지막 글자가 『민』인 사원의
--   사원명, 주민번호, 급여 항목을 조회한다.
...
--○ TBL_SAWON 테이블에서 사원 이름에 『이』라는 글자가
--   하나라도 포함되어 있다면 그 사원의
--   사원번호, 사원명, 급여 항목을 조회한다.
SELECT 사원번호, 사원명, 급여
FROM TBL_SAWON
WHERE 사원 이름에 『이』라는 글자;
...
SELECT 사원번호, 사원명, 급여
FROM TBL_SAWON
WHERE 사원 이름에 『이』라는 글자가 연속으로 두 번;
...
--○ TBL_SAWON 테이블에서 사원의 이름의 두 번째 글자가 『이』인 사원의
--   사원번호, 사원명, 급여 항목을 조회한다.
--   → 불가~!!!
...
--   (지금 당장은 아니더라도...)
--   테이블에서 성 컬럼과 이름 컬럼을 분리하여 구성해야 한다.ㅣ
...
-- ORDER BY 부서번호;
--> EMP 테이블이 갖고있는 테이블의 고유한 컬럼 순서(2 → ENAME, 사원명)가 아니라
--  SELECT 처리되는 두 번째 컬럼(2 → DEPTNO, 부서번호)를 기준으로 정렬되는 것을 확인
...
--  네 번째 파라미터 값은 몇 번째 등장하는 값을 찾을 건지에 대한 설명 (→ 1은 생략 가능)
...
SELECT FILENO"파일번호"
, SUBSTR(REVERSE(FILENAME), 1, \가 있는 자릿수)"파일명"
FROM TBL_FILES;
...
6	GNP.TSET\
7	LQS.ELCARO\
*/ -- 거꾸로 나오니까 전체 다시 REVERSE

SELECT FILENO"파일번호"
, REVERSE(SUBSTR(REVERSE(FILENAME), 1, INSTR(REVERSE(FILENAME),'\',1)-1))"파일명"
-- 거꾸로( 추출 ( 거꾸로(파일명), 첫번째부터,위치찾기(거꾸로(파일명),\이 등장하는 곳, 찾기시작할 위치)뒤에하나제외))
FROM TBL_FILES;
...
--  ③ 남아있는 Byte 공간을 『왼쪽』부터 세 번째 파라미터 값을 채운다.
--  ④ 이렇게 구성된 최종 결과값을 반환한다.



--○ RPAD
--> Byte 를 확보하여 오름쪽부터 문자로 채우는 기능을 가진 함수
...
--> ① 10Byte 공간을 확보한다.                  → 두 번째 파라미터 값에 의해...
--> ② 확보한 공간에 'ORACLE'문자열을 담는다.   → 첫 번째 파라미터 값에 의해...
--  ③ 남아있는 Byte 공간을 『오른쪽』부터 세 번째 파라미터 값을 채운다.
--  ④ 이렇게 구성된 최종 결과값을 반환한다.
