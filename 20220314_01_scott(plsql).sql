...
BEGIN
    ...
    -- 커서 오픈 시 쏟아져나오는 데이터를 처리(반복문을 활용하여 처리)
    LOOP
...
CREATE [OR REPLACE] TRIGGER 트리거명
    [BEFORE | AFTER]
    이벤트1 [OR 이벤트2 [OR 이벤트3]] ON 테이블명     --> 이벤트 : DML 트리거(INSERT, UPDATE, DELETE) ON 테이블명 : 부비트랩 설치하는 테이블
    [FOR EACH ROW [WHEN TRIGGER 조건]]                --> ROW TRIGGER 에만 붙음 cf. STATEMENT TRIGGER
[DECLARE]
    -- 선언 구문;
BEGIN 
    -- 실행 구문;
