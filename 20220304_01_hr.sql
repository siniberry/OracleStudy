...
AND CONSTRAINT TEST7_COL1_PK PRIMARY KEY(COL1);
--          +
ALTER TABLE TBL_TEST7
...
--==>>
/*
HR	SYS_C007066	TBL_TEST8	C	COL3	COL3 BETWEEN 0 AND 100	
HR	SYS_C007067	TBL_TEST8	P	COL1		
*/
...
ADD( CONSTRAINT TESTMEMBER_SID_PK PRIMARY KEY(SID)
   , CONSTRAINT TESTMEMBER_SSN_CK CHECK (주민번호8째자리 1개가 '1' 또는 '2' 또는 '3' 또는 '4'));
...
--    자식테이블(참조하는 컬림이 포함된 테이블)이 생성되어야 한다.
...
--    그리고 이 때, 부모 테이블에는 반드시 PK 또는 UK 제약 조건이 
--    설정된 컬럼이 존재해야 한다.
...
--    FOREIGN KEY 컬럼에 그 값이 입력되어 있는 경우 샂게되지 않는다.
...
ALTER TABLE TBL_EMP1
ADD CONSTRAINT EMP1_JIKWI_ID_FW FOREIGN KEY(JIKWI_ID)
               REFERENCES TBL_JOBS(JIKWI_ID)
               ON DELETE CASCADE;
--==>> Table TBL_EMP1이(가) 변경되었습니다.
...
/*
HR	SYS_C007075	        TBL_EMP1	P	SID		
HR	EMP1_JIKWI_ID_FW	TBL_EMP1	R	JIKWI_ID		CASCADE
*/
...
-- 2. 형식 및 구조
-- ① 컬럼 레벨의 형식
-- 컬럼명 데이터타입 [CONSTRIANT CONSTRAINT명] NOT NULL
...
-- 4. 기존 생성되어 있는 테이블에 데이터가 이미 들어있는 않은 컬럼(→ NULL 인 상태의 컬럼)을
...
5	김태형	무진장 실습중입니다.	                2022-03-04 14:32:41	0	0
*/
