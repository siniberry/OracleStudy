...
가정) 서울 여의도 LG라는 회사에 근무하는 거래처 직원 명당이
      총 100만 명이라고 가정한다.
      (한 행(레코드)은 70Byte 이다.)
...
SELECT A.거래처회사명, A.회사주소, A.회사전화
     , B.거래처직원명, B.직급, B.이메일, B. 휴대폰, B.회사ID
FROM 회사 A, 직원 B
WHERE A.회사ID = B.회사ID
...
가정) 서울 여의도 LG라는 회사에 근무하는 거래처 직원 명당이
      ...
      그러면... 회사 테이블에서 1건의 회사주소와 회사 전화를 변경해야 한다.
...
--   컬럼 1개로만 구성된 PRIMARY KEY를 SINGLE PRIMARY KEY하고 부른다.
--   (단일 프라이머리 키)
...
-- 제 2 정규화
--> 제 1 정규화를 마친 결과물에서 PRIMARY KEY 가 SINGLE COLUMN DLFKAUS
--  제 2 정규화는 수행하지 않는다.
--  하지만 PRIMARY KEY 가 COMPOSITE COLMN이라면
...
-----------------------------------     -------------------------------------------
 ...
 1002     김상지  ksk@t..  010...         wkfkfc   자갈치      500    자갈이 들어...
...
--> 『부서』테이블과 『사원』테이블을 JOIN 했을 때의 크기
--  (500000*30Byte)  +  (1000000*60Byte) = 15000000 + 60000000 = 750000000
...
5. 제약조건의 종류
   ...
   - FOREIGN KEY(FK:F:R) → 외래키, 외부키,  
     해당 컬럼의 값은 참조되는 테이블의 컬럼데이터 중 하나와
     일치하거나 NULL을 가진다.
...
HR	BIN$hgmcgoJCSnqM5rBJk+EX9A==$0	P	BIN$9EOZo5qhRXOSYz9msfLBtw==$0					ENABLED	NOT DEFERRABLE	IMMEDIATE	VALIDATED	GENERATED NAME			2022-03-03	HR	BIN$8vY4flq3Q6eCc0gXvxI/Iw==$0		
HR	SYS_C007056	P	TBL_TEST1					ENABLED	NOT DEFERRABLE	IMMEDIATE	VALIDATED	GENERATED NAME			2022-03-03	HR	SYS_C007056		
*/
...
WHERE TABLE_NAME = 'TBL_TEST1';
--==>> HR	SYS_C007056	TBL_TEST1	COL1	1
--          -----------
--           제약조건
...
HR	BIN$hgmcgoJCSnqM5rBJk+EX9A==$0	BIN$9EOZo5qhRXOSYz9msfLBtw==$0	P	COL2
HR	SYS_C007056	TBL_TEST1	P	COL1
*/
...
--CREATE OR REPLACE VIEW_CONSTCHECK
...
--    PRIMARY KEY와 유사한 제약저간이지만, NULL 을 허용한다는 차이점이 있다.
...
--==>>
/*
HR	SYS_C007060	TBL_TEST5	P	COL1		
HR	SYS_C007061	TBL_TEST5	U	COL2		
*/
